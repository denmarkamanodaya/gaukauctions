...
class DealerCategoryService
{
    ...
    public function getAdminCategoryBySlug($slug)
    {
        return DealerCategories::with('parent')->where('slug', $slug)->whereNull('user_id')->firstOrFail();
    }
    ...
    public function getCategoryBySlug($slug, $area=null)
    {
        if($area)
        {
            return Cache::remember('category_'.$slug.$area, 60, function() use($slug, $area) {
                return DealerCategories::with('parent')->where('slug', $slug)->whereNull('user_id')->whereHas('parent', function($query) use($area) {
                    $query->where('slug', $area);
                })->allowed()->firstOrFail();
            });
        }
        return Cache::remember('category_'.$slug, 60, function() use($slug) {
            return DealerCategories::with('parent')->where('slug', $slug)->whereNull('user_id')->allowed()->firstOrFail();
        });
    }
    ...
    public function updateCategory($request, $slug)
    {
        ...
        $category->children()->update(['hidden' => $request->hidden]);
    ...
    public function cachedCategoryList($except=[], $useHeaders=true)
    {
        $categories = Cache::rememberForever('categoryList', function () use($except) {
            return DealerCategories::allowed()->whereHas('children')->with(['children' => function($query) {
        ...
        {
            if(in_array($category->slug, $except)) continue;
            if($useHeaders)
            {
                $catList[$category->name] = $category->children->pluck('name', 'id');
            } else {
                $catList = $catList + $category->children->pluck('name', 'id')->toArray();
            }
        }
        return $catList;
    }
