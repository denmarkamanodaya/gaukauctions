...
class CalendarImportService
{
    ...
    public function import_list($searchTerm=null, $except=null)
    {
        if($except)
        {
            $posts = Import::whereHas('categories', function ($query) use($searchTerm, $except){
                $query->whereNotIn('name', $except);
            })->with('categories')->where('import_area', 'auctioneer_event')->where('import_type', 'wp')->where('complete', 0)->tenant()->paginate(25);
        } elseif($searchTerm) {
            $posts = Import::whereHas('categories', function ($query) use($searchTerm, $except){
                $query->whereIn('name', $searchTerm);
            })->with('categories')->where('import_area', 'auctioneer_event')->where('import_type', 'wp')->where('complete', 0)->tenant()->paginate(25);
        } else {
            $posts = Import::with('categories')->where('import_area', 'auctioneer_event')->where('import_type', 'wp')->where('complete', 0)->tenant()->orderBy('title', 'ASC')->paginate(25);
        }
